...
class LabNode(Node):
    ...
    def timer_callback(self):
        ...
        if fwd <= IDEAL + 0.6*(MAX - IDEAL):
            ...
            twist.angular.z = FOLLOW_WEIGHT * (theta - (self.right * NORM))
            self.cmd_vel_publisher.publish(twist)
            return
        ...
        if norm < MAX:
            ...
            self.get_logger().info(
                f"follow {FOLLOW_WEIGHT * (theta - (self.right * NORM))} hold {v1}")
        ...
        if self.last_turn and np.abs(twist.angular.z - self.last_turn) > 3 * PI / 4:
