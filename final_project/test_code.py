...
class LabNode(Node):
  ...
  def timer_callback(self):
    ...
    casts = np.convolve(np.concatenate((A[-5:], A, A[:5])), [0.2] * 5, mode = 'valid')
    ...
    if (c_i * TO_RAD - FWD > PI/8 and c_i * TO_RAD - FWD <  3 * PI/4):
      self.get_logger().info("LEFT IDEAL")
      twist.angular.z -= 0.25 * HOLD_WEIGHT * ((close - IDEAL) / PUSH_CONST) ** 2
