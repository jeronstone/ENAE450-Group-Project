...
DELIBERATION = 0.1  # distance of deliberation
IDEAL = 0.15  # ideal wall following radius (m)
MAX = 0.5  # max search radius (m)
SPEED = 0.25  # normal speed (m/s)
...
class LabNode(Node):
    ...
    def timer_callback(self):
        ...
        if casts[np.argmin(casts[(90//2):(330//2)])] - casts[np.argmin(casts[(390//2):(630//2)])] < DELIBERATION:
            # Takes the closest thing on the right half
            c_i = np.argmin(casts[:(360//2)])
        ...
        if close <= DANGER:  # if anything is in our danger zone
            ...
            self.get_logger().info(
                f"DANGER! {twist.linear.x} {twist.angular.z}")
        ...
        if fwd <= IDEAL:
            self.get_logger().info(f'FWD TURN: {fwd}')
        ...
        if norm < MAX:
            ...
            #self.get_logger().info(
            #    f"follow {FOLLOW_WEIGHT * (theta - NORM)} hold {v1}")
        ...
        if np.abs(c_i * TO_RAD - FWD) > PI / 2 and close < IDEAL:
            ...
            twist.angular.z += 0.0 * HOLD_WEIGHT * \
                ((close - IDEAL) / PUSH_CONST) ** 2
