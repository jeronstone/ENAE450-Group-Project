...
class ArucoNode(Node):
    ...
    def frame_handler(self, image_data):
        self.print(f'Got Frame {image_data.header.frame_id} @ {image_data.header.stamp}')

        # get camera frame data
        frame = np.array(image_data.data)

        # detect arucos
        (corners, ids, rejected) = cv2.aruco.detectMarkers(frame, dictionary=dictionary)
